...
from parser_transition import minibatch_parse
...
class Config(object):
    ...
    data_path = './dataset/VnDT'
    train_file = 'VnDTv1.1-predicted-POS-tags-train.conll'
    dev_file = 'VnDTv1.1-predicted-POS-tags-dev.conll'
    test_file = 'VnDTv1.1-predicted-POS-tags-test.conll'
    embedding_file = './dataset/word2vec_vi_syllables_100dims.txt'
...
def load_and_preprocess_data(reduced=True):
    ...
    start = time.time()
    word_vectors = {}
    for line in open(config.embedding_file, 'r', encoding = 'utf-8').readlines():
        sp = line.strip().split()
        if len(sp) != 101:
            continue
        word_vectors[sp[0]] = [float(x) for x in sp[1:]]
    embeddings_matrix = np.asarray(np.random.normal(0, 0.9, (parser.n_tokens, 100)), dtype='float32')
